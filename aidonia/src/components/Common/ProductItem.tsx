...
const ProductItem = ({ item }: { item: ExtendedProduct }) => {
  ...
  // Check if item is already favorited when user is authenticated
  useEffect(() => {
    const checkFavoriteStatus = async () => {
      if (user?.id && item.id) {
        try {
          const result = await checkIfPostFavorited(user.id, item.id);
          ...
          // Silently handle errors for favorite checking to avoid UI disruption
          console.debug("Error checking favorite status:", error);
        }
      }
    };
  ...
  const handleWishlistToggle = async () => {
    ...
    try {
      if (isFavorited && favoriteId) {
        ...
        setIsFavorited(true);
        setFavoriteId(response.data?.id || response.id);
  ...
  // add to cart
  // Removed unused handleAddToCart function to resolve the error
        ...
        <div className="absolute left-0 bottom-0 translate-y-full w-full flex items-center justify-center gap-2.5 pb-5 ease-linear duration-200 group-hover:translate-y-0">
          ...
          >
            {isWishlistLoading ? (
              ...
              <svg
                className={`${isFavorited ? "fill-current" : "fill-none stroke-current stroke-2"}`}
                width="16"
                height="16"
                viewBox="0 0 16 16"
                xmlns="http://www.w3.org/2000/svg"
