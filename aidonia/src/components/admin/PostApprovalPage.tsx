...
export default function PostApprovalPage() {
  ...
  const loadPosts = async (
    ...
    try {
      ...
      const items = Array.isArray(postsResp?.data?.items)
      ...
      if (postsResp.isSuccess || items.length >= 0) {
        ...
        // setTotalItems(postsResp.data?.totalItems ?? null); // totalItems not in type
      } else {
        ...
        setTotalPages(0);
        // setTotalItems(null); // totalItems not in type
      }

      // Fallback counts from the loaded posts if statistical endpoints fail
      const fallbackApproved = items.filter(
        (p) => p.status === "APPROVED"
      ).length;
      const fallbackRejected = items.filter(
        (p) => p.status === "REJECTED"
      ).length;
  ...
  return (
    <div className="mx-auto max-w-full">
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-3 md:gap-6 xl:grid-cols-3 2xl:gap-7.5">
        ...
        <div
          className="rounded-[10px] border px-7.5 py-6 shadow-lg transition-all duration-300"
          style={{
      ...
      <div className="rounded-[10px] border border-stroke bg-white px-7.5 py-6 shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
        ...
        <ConfirmDialog
          ...
          title={
            confirmDialog.type === "approve" ? "Approve Post" : "Reject Post"
          }
          message={
